...
git_list_refs <- function(url, prefixes = NULL) {
  synchronise(async_git_list_refs(url, prefixes))
}
...
git_list_files <- function(url, ref = "HEAD") {
  synchronise(async_git_list_files(url, ref))
}
...
git_download_file <- function(url, sha, output = sha) {
  synchronise(async_git_download_file(url, sha, output))
}
...
git_fetch <- function(url, sha, blobs = FALSE) {
  synchronise(async_git_fetch(url, sha, blobs))
}
...
git_download_repo <- function(url, ref = "HEAD", output = ref) {
  synchronise(async_git_download_repo(url, ref, output))
}
...
git_send_message_v2 <- function(
  ...
  synchronise(async_git_send_message_v2(url, cmd, caps = caps, args = args)) # nocov
}
...
git_list_refs_v1 <- function(url) {
  synchronise(async_git_list_refs_v1(url))
}
...
git_list_refs_v2 <- function(url, prefixes = character()) {
  synchronise(async_git_list_refs_v2(url, prefixes))
}
