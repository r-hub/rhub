...
If you are new to Docker, for the basic use shown in the next two sections you 
don't need to learn anything, you won't have to leave R. 
Nonetheless, if you're curious, this tutorial [features a nice introduction](https://ropenscilabs.github.io/r-docker-tutorial/01-what-and-why.html). 
Also see [this blog post](https://colinfay.me/docker-r-reproducibility/) and 
the list of resources it shows at the end.
...
including links to the built images on Docker Hub)._
...
The object returned is of the class `rcmdcheck::rcmdcheck` which is an S3 object 
with fields errors, warnings and notes (character vectors), that you could 
operate on if you wish.

The `local_check_linux()` function creates a container (instance of the image) that 
won't be deleted after use so you might want to clean up once in a while.

## Do more with R-hub Linux images

If running checks in images iteratively isn't enough for your debugging, you 
might want to run the container created by `local_check_linux()`. Take note of 
the container name and run (in a shell, not in R)
...
where `7181196d-bc3c-4fc8-a0e8-dc511150335d-2` is the container name. Thanks 
to these commands, you will be in a shell in the Docker images, where you can 
run R.
...
Or you could create a new container from the image.

To know how to build the package, install the dependencies, etc. you should have a look at `rhub` Docker bash script. To find it, run the code below.

```{r}
system.file(package = "rhub", "bin", "rhub-linux-docker.sh")
```

More Docker guidance is outside the scope of this vignette.