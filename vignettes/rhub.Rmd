...
by default, your R session is busy showing the log until the build is finished, 
...
To retrieve such objects from previous builds, see [the corresponding section](#browse-previous-builds).
...
You can then extract builds from this list using their `group` ID (ID for 
builds submitted simultaneously e.g. via `check_for_cran()`) or `id` (check id).
...
```r
group_id <- previous_builds$group[1]
